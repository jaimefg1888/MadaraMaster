#!/usr/bin/env python3
# MadaraMaster — motor de borrado seguro (DoD 5220.22-M)
# jaimefg1888
#
# Tres pases de sobrescritura:
#   Pase 1: Ceros      (\x00)
#   Pase 2: Unos       (\xFF)
#   Pase 3: Aleatorio  (os.urandom)
#
# Anti-forense: renombra el archivo y resetea timestamps antes de eliminar
...
@dataclass
class WipeTelemetry:
    # snapshot en tiempo real para actualizar el dashboard
...
def _scrub_metadata(filepath: str) -> str:
    # resetea timestamps y renombra a nombre aleatorio antes de eliminar
    try:
...
def wipe_file(filepath: str, progress_callback: ProgressCallback = None) -> WipeResult:
    # borrado con DoD 5220.22-M (3 pases) + limpieza de metadatos
...
def wipe_directory(dirpath: str, progress_callback: ProgressCallback = None) -> WipeSummary:
    # borra recursivamente todos los archivos de un directorio
    summary = WipeSummary()
    start_time = time.time()
    ...
    # eliminar directorios vacíos de abajo a arriba
...
def collect_files(target: str) -> List[str]:
    # devuelve lista de rutas absolutas de archivos dentro del objetivo
