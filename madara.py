...
LANG = {
    "EN": {
        ...
        "session_hint":       "Drag a file or folder and press Enter.",
        "session_exit_hint":  "Type [bold]exit[/bold] or [bold]close[/bold] to quit.",
        "session_prompt":     "❱❱❱ Drag and drop file/folder or enter path (or 'exit'): ",
    ...
    "ES": {
        # -- Interactive session --
        "session_title":      "Modo Sesión Interactivo",
        "session_hint":       "Arrastra un archivo o carpeta y pulsa Enter.",
        "session_exit_hint":  "Escribe [bold]exit[/bold] o [bold]close[/bold] para salir.",
        "session_prompt":     "❱❱❱ Arrastre archivo/carpeta o introduzca la ruta (o 'salir'): ",
...
# Exit keywords (works in both languages)
EXIT_KEYWORDS = frozenset(["exit", "close", "quit", "salir", "cerrar"])
...
BANNER = """
 ...
 ╚═╝     ╚═╝╚═╝  ╚═╝╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝
   ...
   MadaraMaster v2.1 • Created by jaimefg1888 • DoD 5220.22-M
"""
...
def interactive_session():
    ...
    print_banner()
    console.print(
        f"  [bold cyan]{T('session_title')}[/] — {T('session_hint')}"
    )
    ...
    while True:
        ...
        if cleaned.lower() in EXIT_KEYWORDS:
        ...
        # Invoke the wipe command via sys.argv
        sys.argv = ["madara.py", "wipe", target]
        try:
            app(standalone_mode=False)
        except SystemExit:
            pass

        console.print()  # Visual separator between operations
